...
export default class Db extends EventEmitter {
    _cols: any = {};
    constructor(private _name: string, private _version?: number, private _config: any = {}) {
        super();
        ...
        if (Array.isArray(_config)) {
            for (let name of _config) {
                this._addCollection(name);
                this._config[name] = true;
            }
        } else {
            for (let name in _config) {
                this._addCollection(name);
                this._addIndex(_config[name], name);
            }
        }
    }
